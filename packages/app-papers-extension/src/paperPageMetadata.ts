...
export const readPaperPageMetadata = (doc: Document = document): BrowserDetectedPaperSource => {
  ...
  return {
    ...
    doi: queryMeta(doc, ['citation_doi', 'dc.identifier', 'DC.Identifier']) ?? identifiers.doi,
    arxivId: identifiers.arxivId,
    pdfUrl: queryMeta(doc, ['citation_pdf_url']) ?? identifiers.pdfUrl,
    title: queryMeta(doc, ['citation_title', 'dc.title', 'DC.Title']) ?? doc.title,
    authors: queryMetaAll(doc, 'citation_author'),
