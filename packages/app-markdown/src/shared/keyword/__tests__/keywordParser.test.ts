...
describe('keywordParser', () => {
  it('parses keyword tags and attributes', () => {
    const keywords = parseKeywords(
      '<keyword name="K1">body</keyword><keyword noindex number-class="1" prefix="Theorem">x</keyword>'
    );
  ...
  it('injects generated names for number-class keywords', () => {
    ...
    const result = injectKeywordNames(
      '<keyword number-class="2" prefix="Theorem">body</keyword>',
      (numberClass, prefix) => {
        count += 1;
    ...
    expect(result).toContain('name="Theorem 2.1"');
  });
});
